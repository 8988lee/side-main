...
async function fetchData1() {
  ...
  const response = await fetch(
    "http://www.secondlearner.com/gchrome/jsonCaption.php"
  );

  const data = await response.json();

  const display = document.querySelector("#captionFrom1");
...
document
  .getElementById("fetchCaptionbtnGET1")
  .addEventListener("click", fetchData1);
